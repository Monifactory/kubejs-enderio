...
public class CustomEnergyConduitData implements ConduitData<CustomEnergyConduitData> {
    ...
    private LazyOptional<IEnergyStorage> selfCap = LazyOptional
        .of(() -> new CustomEnergyConduitData.ConduitEnergyStorage(this));

    public CustomEnergyConduitData(int transferRate) {
        this.capacity = transferRate;
    }
    @Override
    public CompoundTag serializeNBT() {
        ...
        {
            @Nullable
            EnergySidedData sidedData = energySidedData.get(direction);
            if (sidedData != null)
            {
    ...
    public void deserializeNBT(CompoundTag nbt) {
        ...
        for (Direction direction : Direction.values())
        {
            ...
            {
                energySidedData.put(direction,
                    EnergySidedData.fromNbt(nbt.getCompound(direction.name())));
            }
        }
    ...
    LazyOptional<IEnergyStorage> getSelfCap() {
        ...
        {
            selfCap = LazyOptional.of(() -> new CustomEnergyConduitData.ConduitEnergyStorage(this));
        }
        return selfCap;
    }
    ...
    private record ConduitEnergyStorage(CustomEnergyConduitData data) implements IEnergyStorage {
